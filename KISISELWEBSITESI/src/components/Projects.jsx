...
function Projects() {
  ...
  return (
    <section
      className={`w-full py-20 transition-colors duration-300 ${darkMode ? "bg-[#1a161f]" : "bg-[#CBF281]"}`}
    >
      <div className="w-full max-w-7xl mx-auto px-6 relative">
        {/* Başlık ve Oklar */}
        <div className="flex justify-between items-center mb-12">
          <h2
            className={`text-4xl md:text-5xl font-extrabold ${darkMode ? "text-[#CBF281]" : "text-[#4731D3]"}`}
          >
          ...
          <div className="flex gap-4">
            <button
              onClick={() => scroll("left")}
              className="p-3 bg-[#4731D3] text-white rounded-full hover:bg-opacity-80 transition shadow-lg"
            >
              ←
            </button>
            <button
              ...
              <div className="p-8 flex flex-col grow">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies?.map((tech, i) => (
                    <span
                      key={i}
                      className="px-3 py-1 bg-[#4731D3] text-white text-[10px] font-bold rounded-full"
                    >
                      {tech.toUpperCase()}
                    </span>
                  ))}
