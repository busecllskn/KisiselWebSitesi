...
function Footer() {
  ...
  const handleSubmit = (e) => {
    ...
    alert("Kayıt işleminiz başarıyla halloldu, artık iletişime geçebiliriz!");
      ...
      <div className="max-w-4xl mx-auto px-6 flex flex-col items-center text-center space-y-6">
        ...
        {isFormOpen && (
          <form onSubmit={handleSubmit} className="w-full max-w-sm flex flex-col gap-4 animate-in fade-in duration-300">
            <input name="name" value={values.name} onChange={handleChange} placeholder="İsim (5-30 karakter)" className="p-3 rounded-lg border w-full text-black" required />
            ...
            <input name="password" type="password" value={values.password} onChange={handleChange} placeholder="Şifre (en az 6 karakter)" className="p-3 rounded-lg border w-full text-black" required />
        ...
        {/* İletişim Bilgileri (Form kapalıyken görünür) */}
        {!isFormOpen && (
          <>
